...
⚡ HTTP server framework built for high performance and portability across runtimes.
...
```typescript
import { createApp } from '@wpackages/http-server'
...
```typescript
import { createApp, createRouter, defineEventHandler } from '@wpackages/http-server';
...
```typescript
import { createMiddleware } from '@wpackages/http-server';
...
```typescript
import { validateString, createValidator } from '@wpackages/http-server';
...
```typescript
import { createPluginManager, corsPlugin } from '@wpackages/http-server';
...
| Feature | Http Server | Express | Fastify | Hono |
|----------|---------|---------|---------|------|
...
- `@wpackages/webserver` - Advanced web server framework  
