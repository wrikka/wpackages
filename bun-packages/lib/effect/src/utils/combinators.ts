...
export const flatMap =
	<A, B, E2, R2>(f: (a: A) => Effect<B, E2, R2>) => <E, R>(effect: Effect<A, E, R>): Effect<B, E | E2, R | R2> => {
		return (async () => {
			const value = await effect();
...
export const tap =
	<A, E2, R2>(f: (a: A) => Effect<void, E2, R2>) => <E, R>(effect: Effect<A, E, R>): Effect<A, E | E2, R | R2> => {
		return (async () => {
			const value = await effect();
			await f(value as A)();
			return value;
...
export const all = <const Effects extends readonly Effect<any, any, any>[]>(
	...
	{
		[K in keyof Effects]: Effects[K] extends Effect<infer A, any, any> ? A
			: never;
	},
...
export const allSuccesses = <
	const Effects extends readonly Effect<any, any, any>[],
>(
	effects: Effects,
): Effect<
	{
...
> => {
	return (async () => {
		const results: (Effects[number] extends Effect<infer A, any, any> ? A
			: never)[] = [];
...
export const forEach = <A, B, E, R>(f: (a: A) => Effect<B, E, R>) => (items: readonly A[]): Effect<B[], E, R> => {
	return (async () => {
		const results: B[] = [];
...
> => {
	return (async () => {
		...
		)) as Effects[number] extends Effect<infer A, any, any> ? A : never;
	}) as Effect<
		Effects[number] extends Effect<infer A, any, any> ? A : never,
		Effects[number] extends Effect<any, infer E, any> ? E : never,
		Effects[number] extends Effect<any, any, infer R> ? R : never
