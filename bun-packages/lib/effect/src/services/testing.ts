...
export const createSpy = <A, E>(
	_effect: Effect<A, E>,
): {
	_tag: "Spy";
	calls: Array<{ args: unknown[]; result: A | E }>;
...
export const createStub = <A, E>(
	_returnValue?: A,
): {
	_tag: "Stub";
	calls: Array<{ args: unknown[]; result: A | E }>;
	withReturnValue: () => void;
	returnValue: () => void;
	withError: () => void;
	thenReturnError: () => void;
} => {
	const calls: Array<{ args: unknown[]; result: A | E }> = [];
...
export const assertCalledWith = <A, E>(
	spy: Spy<A, E>,
	...args: unknown[]
): void => {
...
export const assertCalledTimes = <A, E>(
	spy: Spy<A, E>,
	times: number,
): void => {
	if (spy.getCalls().length !== times) {
		throw new Error(
			`Expected spy to be called ${times} times, but was called ${spy.getCalls().length} times`,
		);
	}
};
