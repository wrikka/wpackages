 ...
 */
export const formatHelpHeader = (title: string): string => `${pc.bold(pc.cyan(title))}`;

/**
 * Format help section
 */
export const formatHelpSection = (title: string, content: string): string => `${pc.bold(title)}\n${content}`;
 ...
 */
export const formatTableSeparator = (widths: number[]): string => widths.map((w) => "─".repeat(w)).join("─┼─");
