...
import { collectSummary, computeBuildHash, loadCacheHash, saveCacheHash } from "..";
import { buildBin, buildNative, buildTS, buildWasm, emitDts } from "./builder.service";
...
export class BuildOrchestrator {
	...
	public static async create(
		inlineConfig: Partial<BunpackConfig> = {},
	...
	private async runHook(name: keyof Hooks) {
		...
		if (hook) {
			if (name === "configResolved") {
				await (hook as (config: BunpackConfig) => void | Promise<void>)(
					this.context.resolvedConfig,
				);
			} else {
	...
	private async performBuild() {
		...
		if (shouldRunClean(this.config) && this.config.outDir) {
			...
			if (!policy?.steps || policy.steps.length === 0) {
				console.log(
					`Cleaning output directory: ${pc.yellow(this.config.outDir)}`,
				);
				await rm(this.config.outDir, { recursive: true, force: true });
			} else {
				console.log(
					`Cleaning output directory (policy): ${pc.yellow(this.config.outDir)}`,
				);
				for (const f of formats) {
					await rm(path.join(this.config.outDir, f), {
						recursive: true,
						force: true,
					});
				}
				await rm(path.join(this.config.outDir, "types"), {
					recursive: true,
					force: true,
				});
				await rm(path.join(this.config.outDir, "bin"), {
					recursive: true,
					force: true,
				});
			}
	...
	public async build() {
		console.log(
			pc.green(
				`Starting build for ${pc.bold(this.config.name ?? "unnamed package")}...`,
			),
		);
