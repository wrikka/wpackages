...
import { cleanup, findCleanableItems, type CleanableItem } from './services/cleanup.service'
...
export const runCleanupApp = async () => {
    ...
    for (const basePath of config.scanPaths) {
        ...
        const itemsInPath = await findCleanableItems(config.patterns, basePath, config.excludePatterns);
    ...
    if (p.isCancel(selectedItems) || !Array.isArray(selectedItems) || selectedItems.length === 0) {
      p.cancel('Operation cancelled. No files were cleaned.')
      return
    }

    const totalSize = selectedItems.reduce((acc, item) => acc + (item as CleanableItem).size, 0)
    ...
    try {
      const pathsToClean = selectedItems.map(item => (item as CleanableItem).path)
        ...
        p.outro(`Cleanup finished. ${summary.join(', ')}. Total space saved: ${pc.green(formatBytes(totalSize))}`)
