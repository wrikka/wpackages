...
export const loadConfig = async (): Promise<CleanupConfig> => {
  try {
    ...
    const allPatterns = [...new Set([...defaultConfig.patterns, ...(userConfig.patterns || [])])];
    // User-defined scanPaths will override the default
    const scanPaths = userConfig.scanPaths && userConfig.scanPaths.length > 0 ? userConfig.scanPaths : defaultConfig.scanPaths;
    const allExcludePatterns = [...new Set([...defaultConfig.excludePatterns, ...(userConfig.excludePatterns || [])])];
    return { ...defaultConfig, ...userConfig, patterns: allPatterns, scanPaths, excludePatterns: allExcludePatterns };
  } catch {
