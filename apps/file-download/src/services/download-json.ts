...
export async function downloadJson() {
	const options = await p.group(
		{
			...
			output: () => p.text({ message: 'Enter the output filename (e.g., data.json):' }),
		},
		{
			onCancel: () => {
	...
	try {
		const response = await axios.get(parsedOptions.data.url, { responseType: 'json' });
		await writeFile(parsedOptions.data.output, JSON.stringify(response.data, null, 2));
