...
//!
//! The CommandPalette provides a searchable interface for executing commands
//! in the TUI application. It features:
//!
//! - Fuzzy search for quick command discovery
//! - Command categorization for better organization
//! - Command history for quick access to recent commands
...
//!
//! if let Some(cmd) = palette.get_selected_command() {
//!     println!("Selected: {}", cmd.name);
//! }
//! ```

use crate::components::ui::theme::Theme;
use ratatui::{layout::Rect, widgets::ListState, Frame};

#[path = "command_defaults.rs"]
mod command_defaults;
#[path = "command_filter.rs"]
mod command_filter;
#[path = "command_fuzzy.rs"]
mod command_fuzzy;
#[path = "command_history.rs"]
mod command_history;
...
pub use command_types::{Command, CommandCategory};

use command_fuzzy::FuzzyMatch;
...
impl CommandPalette {
    ...
    pub fn execute_selected(&mut self) -> Option<String> {
        if let Some(cmd) = self.get_selected_command() {
            let cmd_id = cmd.id.as_str().to_string();
            self.add_to_history();
            Some(cmd_id)
        } else {
            None
