...
use crate::types::{ExecutionPlan, PlanStatus, ExecutionMode};
...
impl PlanPanel {
    ...
    fn render_plan(&self, frame: &mut Frame, area: Rect, block: Block, plan: &ExecutionPlan) {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(format!("{:?}", plan.status), Style::default().fg(status_color)),
        ]));
        lines.push(Line::default());
        ...
        for (i, step) in plan.steps.iter().enumerate() {
            ...
            let approval_color = if step.approved { Color::Green } else { Color::Gray };
