...
<template>
	<div class="bg-gray-900 text-gray-100 min-h-screen font-sans">
		...
		<main class="container mx-auto px-4 py-8">
			<div class="bg-gray-800 p-4 rounded-lg mb-6 border border-gray-700 flex justify-between items-center">
				<div>
					...
					<p class="text-gray-400 text-sm">
						Actively polling for new traces...
					</p>
				</div>
			...
			<div
				v-if="error"
				class="bg-red-900/50 border border-red-700 text-red-300 p-4 rounded-lg"
			>
			...
			<div v-else-if="traces.length === 0" class="text-center py-16">
				...
				<p class="text-sm text-gray-600 mt-2">
					Ensure your application is running and configured with the
					WebUiExporter.
				</p>
			</div>
			...
			<div v-else class="space-y-4">
				<TraceWaterfall
					v-for="(trace, index) in groupedTraces"
					:key="index"
					:trace="trace"
				/>
			</div>
