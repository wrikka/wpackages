...
import { getRepoFiles, commitAndPushFiles, generateCommitMessage } from '@/services';
...
async function main() {
  ...
  const validFiles = filesToCommit.filter(f => f !== null) as { path: string; content: string }[];
  ...
  const diff = validFiles.map(f => `--- a/${f.path}\n+++ b/${f.path}\n${f.content}`).join('\n');
  ...
  if (shouldCommit) {
    ...
    try {
      const commitSha = await commitAndPushFiles(owner, repo, validFiles, commitMessage);
      cli.stopSpinner(commitSpinner, `Successfully committed and pushed. Commit SHA: ${commitSha}`);
    } catch (error: any) {
      cli.stopSpinner(commitSpinner, `Failed to commit and push files: ${error.message}`);
    }
  }
