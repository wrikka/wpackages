...
const colorIndex = Math.abs(hashCode(props.span.resource?.attributes?.['service.name'] ?? props.span.name)) % 5;
const colors = [
...
<template>
  ...
  >
    ...
    <span class="ml-2 text-gray-400 font-mono">{{ duration.toFixed(2) }}ms</span>
  </div>
</template>
