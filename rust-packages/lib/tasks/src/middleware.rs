...
pub type TaskHandler = Box<dyn Fn(Task) -> Pin<Box<dyn Future<Output = Result<TaskResult>> + Send>> + Send + Sync>;
...
impl Middleware for LoggingMiddleware {
    ...
    fn after(&self, task: &Task, result: &mut TaskResult) -> Result<()> {
        if result.success {
            tracing::info!("Task completed: {} (id: {}) in {}ms", task.name, task.id, result.duration_ms);
        } else {
            tracing::error!("Task failed: {} (id: {}) - {:?}", task.name, task.id, result.error);
        }
        Ok(())
    }
...
impl MetricsMiddleware {
    ...
    pub fn new() -> Self {
        ...
        let tasks_failed = prometheus::register_int_counter!(
            "task_failed_total",
...
impl Middleware for MetricsMiddleware {
    ...
    fn after(&self, task: &Task, result: &mut TaskResult) -> Result<()> {
        ...
        self.task_duration.observe(result.duration_ms as f64 / 1000.0);
        Ok(())
    }
}
...
impl Middleware for RetryMiddleware {
    ...
    fn on_error(&self, task: &Task, _error: &crate::error::TaskError) -> Result<()> {
        if task.can_retry() {
            tracing::warn!("Task {} will be retried (attempt {}/{})", task.id, task.retry_count + 1, task.max_retries);
        }
        Ok(())
    }
}
