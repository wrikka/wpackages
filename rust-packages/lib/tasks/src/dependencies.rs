...
impl<S: TaskStore> DependencyManager<S> {
    ...
    pub async fn add_dependencies(&self, task_id: &str, dependencies: Vec<TaskDependency>) -> Result<()> {
        ...
        let deps: Vec<TaskDependency> = serde_json::from_value(
            task.metadata.get("dependencies")
                .cloned()
        ...
        task.metadata["dependencies"] = serde_json::to_value(&all_deps)
    ...
    pub async fn can_run(&self, task_id: &str) -> Result<bool> {
        ...
        for dep in dependencies {
            let dep_task = self.store.get_task(&dep.task_id).await?
                .ok_or_else(|| TaskError::Other(format!("Dependency task {} not found", dep.task_id)))?;
            ...
            match dep.dependency_type {
                ...
                DependencyType::Completion => {
                    if !matches!(dep_task.status, TaskStatus::Completed | TaskStatus::Failed | TaskStatus::Cancelled) {
                        return Ok(false);
                    }
                }
    ...
    pub async fn detect_cycles(&self, task_id: &str) -> Result<bool> {
        ...
        for dep in dependencies {
            if self.has_cycle(task_id, &dep.task_id, &mut HashSet::new()).await? {
                return Ok(true);
            }
        }
    ...
    async fn has_cycle(&self, start: &str, current: &str, visited: &mut HashSet<String>) -> Result<bool> {
        if current == start {
            return Ok(true);
        }
