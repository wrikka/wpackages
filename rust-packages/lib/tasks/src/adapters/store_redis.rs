...
impl RedisTaskStore {
    pub async fn new(redis_url: &str) -> Result<Self> {
        let client = Client::open(redis_url)
            .map_err(|e| TaskError::Other(format!("Redis connection error: {}", e)))?;
...
impl TaskStore for RedisTaskStore {
    async fn save_task(&self, task: &Task) -> Result<()> {
        ...
        if task.status == TaskStatus::Pending {
            ...
            let _: () = self
                .conn
                .zadd(&priority_key, &task.id, task.created_at.timestamp())
                .await
                .map_err(TaskError::Database)?;
        }
        ...
        if let Some(scheduled_at) = task.scheduled_at {
            ...
            let _: () = self
                .conn
                .zadd(Self::scheduled_queue_key(), &task.id, score)
                .await
                .map_err(TaskError::Database)?;
        }
    ...
    async fn get_task(&self, task_id: &str) -> Result<Option<Task>> {
        ...
        let task_json: Option<String> = self
            .conn
            .get(&task_key)
            .await
            .map_err(TaskError::Database)?;
    ...
    async fn delete_task(&self, task_id: &str) -> Result<()> {
        ...
        if let Some(task) = task {
            ...
            if task.scheduled_at.is_some() {
                let _: () = self
                    .conn
                    .zrem(Self::scheduled_queue_key(), task_id)
                    .await
                    .map_err(TaskError::Database)?;
            }
        ...
        match result_json {
            Some(json) => {
                let result: TaskResult =
                    serde_json::from_str(&json).map_err(TaskError::Serialization)?;
                Ok(Some(result))
            }
            None => Ok(None),
        }
