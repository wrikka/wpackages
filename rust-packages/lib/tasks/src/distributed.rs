...
impl<S: TaskStore> DistributedExecutor<S> {
    ...
    /// Try to become leader
    async fn try_become_leader(&self, lease_keepalive: &mut etcd_client::LeaseKeepAlive) -> Result<()> {
        // Create a lease
        let lease = self.etcd_client.lease_grant(self.lease_ttl, None).await
            .map_err(|e| TaskError::Other(format!("Failed to grant lease: {}", e)))?;

        // Start keeping the lease alive
        let (mut keeper, mut responses) = lease_keepalive.keep_alive(lease.id()).await
        ...
        let resp = self.etcd_client.get(self.leader_key.clone(), None).await
    ...
    async fn watch_leader_changes(&self) -> Result<()> {
        let (mut watcher, mut stream) = self.etcd_client
    ...
    async fn process_pending_tasks(&self) -> Result<()> {
        ...
        for task in pending_tasks {
            if task.should_run(chrono::Utc::now()) {
                ...
                // Execute task (implementation depends on your use case)
                // This is a placeholder - you need to implement actual task execution
            }
        }
    ...
    async fn process_scheduled_tasks(&self) -> Result<()> {
        ...
        let scheduled_tasks = self.store.list_scheduled_tasks(now).await?;

        for task in scheduled_tasks {
            info!("Processing scheduled task: {}", task.id);
            // Execute task (implementation depends on your use case)
            // This is a placeholder - you need to implement actual task execution
        }
