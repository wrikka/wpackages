...
export const TerminalLive: Layer.Layer<Terminal> = Layer.scoped(
	Terminal,
	Effect.gen(function* (_) {
		...
		const keyQueue = yield* _(Queue.unbounded<Key>());
		...
		return {
			getSize: Effect.sync(getSize),
			clear: write(ANSI.CLEAR_SCREEN),
			write,
			...
			render: (content: string) =>
				Effect.gen(function* (_) {
					yield* _(write(ANSI.CLEAR_SCREEN));
					yield* _(write(ANSI.moveCursor(1, 1)));
					yield* _(write(content));
				}),
			readKey: Queue.take(keyQueue),
			cleanup: Effect.gen(function* (_) {
				yield* _(disableRawMode());
				yield* _(write(ANSI.SHOW_CURSOR));
				yield* _(Effect.sync(() => rl.close()));
			}),
		};
	}),
);
