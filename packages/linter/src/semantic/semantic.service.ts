...
import { SemanticRule, SemanticError, SemanticLinterError } from '../types/semantic';
...
const readTsConfig = (configPath: string): Effect.Effect<{ config: any; error: ts.Diagnostic | undefined }, SemanticLinterError> =>
    Effect.tryPromise({
...
const loadSemanticRules = (rulesDir: string): Effect.Effect<SemanticRule[], SemanticLinterError> => 
    FileSystemService.readdir(rulesDir).pipe(
        Effect.mapError(cause => new SemanticLinterError({ message: `Failed to read rules directory: ${rulesDir}`, cause })),
...
export const runSemanticLinter = (projectPath: string): Effect.Effect<SemanticError[], SemanticLinterError> => Effect.gen(function* (_) {
    const configPath = yield* _(findTsConfig(projectPath));
    const configFile = yield* _(readTsConfig(configPath));

        if (configFile.error) {
        return yield* _(Effect.fail(new SemanticLinterError({ message: `Error reading tsconfig: ${configFile.error.messageText}` })));
    }
