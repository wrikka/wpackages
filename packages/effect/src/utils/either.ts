...
export const flatMapEither = <L, R, R2>(
	f: (r: R) => Either<L, R2>,
) => (either: Either<L, R>): Either<L, R2> => {
	return isRight(either) ? f(either.right) : either;
};

export const foldEither = <L, R, B>(onLeft: (left: L) => B, onRight: (right: R) => B) => (
	either: Either<L, R>,
): B => {
...
export const fromResult = <E, A>(result: { _tag: "Success"; value: A } | { _tag: "Failure"; error: E }): Either<E, A> => {
	return result._tag === "Success" ? right(result.value) : left(result.error);
};

export const toResult = <L, R>(either: Either<L, R>): { _tag: "Success"; value: R } | { _tag: "Failure"; error: L } => {
	return isRight(either) ? { _tag: "Success", value: either.right } : { _tag: "Failure", error: either.left };
};
