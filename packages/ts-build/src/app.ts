...
import type { BunpackConfig, Target, Format } from "./types";
...
async function buildNative(config: BunpackConfig) {
    ...
    if (!outFile) {
        throw new Error("Could not determine native addon output file name. Please specify 'native.napi.outFile' or 'name' in your config.");
    }
...
async function emitDts(config: BunpackConfig) {
    ...
    const args = [
        'tsc',
        ...(Array.isArray(entry) ? entry.filter((e): e is string => e !== undefined) : [entry].filter((e): e is string => e !== undefined)),
        '--declaration',
        '--emitDeclarationOnly',
        '--target',
        'ES2022',
