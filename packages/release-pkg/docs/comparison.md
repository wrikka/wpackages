...
_Note: Quantitative data like Bundle Size and Dependencies are based on publicly available information (e.g., Bundlephobia, `package.json`) and may vary slightly between versions._
...
¹_Dependency counts reflect direct dependencies listed in `package.json`. The full dependency tree can be significantly larger (e.g., `semantic-release` has been noted to have over 400 transitive dependencies). `@wpackages/release-pkg` maintains a flat dependency structure with no transitive dependencies._
