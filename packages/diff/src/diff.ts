import { isEqual, isObjectLike, type IsEqualOptions } from "./isEqual";
import { lcs } from "./lcs";
...
function diffInternal(expected: unknown, actual: unknown, seen: Seen, options: DiffOptions, path: string[]): DiffResult | undefined {
...
function diffObjects(expected: Record<string, any>, actual: Record<string, any>, seen: Seen, options: DiffOptions, path: string[]): DiffResult | undefined {
    ...
    if (Object.keys(result.added).length === 0 && Object.keys(result.deleted).length === 0 && Object.keys(result.updated).length === 0) {
        return undefined;
    }
	return result;
}

function diffMaps(expected: Map<any, any>, actual: Map<any, any>, seen: Seen, options: DiffOptions, path: string[]): DiffResult | undefined {
    const result: DiffResult = { added: {}, deleted: {}, updated: {} };
