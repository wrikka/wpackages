...
export const Modal = (props: ModalComponentProps): ReturnType<typeof h> => {
	const {
		title = "Modal",
		isOpen = true,
		width = 50,
	...
	return h(
		...
		h(
			...
			`${MODAL_BORDERS.cornerTL}${MODAL_BORDERS.horizontal}${MODAL_BORDERS.cornerTR}`,
		),
		h(
			"text",
			{ color: borderColor },
			`${MODAL_BORDERS.vertical}${titleText}${MODAL_BORDERS.vertical}`,
		),
		h(
			"text",
			{ color: borderColor },
