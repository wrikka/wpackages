...
import { type DataLoader, type QueryKey, type QueryOptions, type QueryState, serializeQueryKey } from "./types";
...
export class Query<T = unknown> {
	...
	constructor(
		...
		this.serializedKey = serializeQueryKey(key);
		this.options = {
	...
	async fetch(): Promise<T> {
		// Check cache first
		const cached = this.client.getCache().get<T>(this.serializedKey);
		...
		while (attempt < maxAttempts) {
			try {
				...
				// Cache the result
				this.client.getCache().set(this.serializedKey, data);
	...
	async mutate(mutator: (data: T | undefined) => T | Promise<T>): Promise<T> {
		...
		try {
			...
			this.client.getCache().set(this.serializedKey, newData);
	...
	invalidate(): void {
		this.client.getCache().delete(this.serializedKey);
		this.fetch();
	}
